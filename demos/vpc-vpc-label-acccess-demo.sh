...
In this demo we will show how to enable connectivity between applications in different networking
domains using MyCelium: Application WAN Interface.
...
read -p ""
read -p "We have development and staging VPCs connected, but default access is set to deny all traffic"
...
read -p "In our demo we will enable connectivity between database instances from those VPCs"
read -p "At first, we will try to ping development database VM from staging database and staging dashboard"
read -p "Ping failed, because default access is set to deny all traffic"
clear
read -p "Now we will request connectivity between staging and development database instances with myCelium CLI using AWI interface."
...
pe "./myCelium  create app-connection  --connection-config examples/vpc-vpc-label-app-demo.yaml"
read -p ""
...
read -p "As expected we can now ping those instances"
clear
read -p "In this demo we showed how to create application connection between instances in different network domains using AWI CLI"
