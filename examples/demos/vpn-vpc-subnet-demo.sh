...
1. Devops admin creates VPC (staging) with three instances (two databases and dashboard) on AWS using Terraform.

2. Checks connectivity between router and VPC – not connected.

3. Devops admin uses AWI CLI (with vManage Plugin) to request connectivity between subnet and VPN.

4. vManage provisions connectivity on behalf of the devops admin.

5. After connectivity is provisioned, router can reach all instances.

6. Checks connectivity between IT application VM and VPC – not reachable due to 'deny all' policy.

7. Devops admin uses AWI CLI (with vManage Plugin) to request connectivity to subnet.

8. After connectivity is provisioned, IT application VM can reach instances in subnet but not the other one.

EOM

read -p ""
echo "For this demo we have pre-created an AWS VPC and an EC2 instances using terraform."
read -p ""
clear
echo "Showing available VPNs"
read -p ""
pe "./awi list vpn"
echo "Showing the AWS VPC Ids"
read -p ""
pe "./awi list vpc-tag --cloud aws"
echo "Showing the AWS VPC Subnets used in demo"
read -p ""
pe "./awi list subnet --cloud aws --tag 'staging*'"
...
echo "We will try to reach the EC2 instances from router using vrf 10."
read -p ""
echo "We can't connect to EC2 instance from router."
...
pe "./awi connect --connection-config vpn-vpc.yaml"
pe "./awi list connection"
echo "We will try to reach the EC2 instances from router using vrf 10."
read -p ""
echo "We will try to reach the EC2 instance with requested subnet from other instance connected to router"
read -p ""
echo "We can't connect to instance due to 'deny' policy."
read -p "We will request connectivity for subnet."
pe "cat vpn-vpc-subnet-acl.yaml"
read -p ""
clear
read -p "Running AWI CLI ..."
pe "./awi connect --connection-config vpn-vpc-subnet-acl.yaml"
echo "We will try again to reach the EC2 instance with requested subnet."
pe "./awi list instance --cloud aws --tag 'database*'"
read -p ""
echo "Connections have been established."
echo "Now we will try to reach the EC2 instance without requested subnet."
read -p ""
echo "We can't connect to instance due to 'deny' policy."
read -p ""
