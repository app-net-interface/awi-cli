...
3. Devops admin uses AWI CLI (with vManage Plugin) to request connectivity to VPC with 'deny all' policy.
...
7. Devops admin uses AWI CLI (with vManage Plugin) to request connectivity to 'database' app.

8. After connectivity is provisioned, IT application VM can reach database instances but not the other one.
...
echo "We will try to reach the EC2 instances from router using vrf 10."
read -p ""
echo "We will try to reach the EC2 instance with requested label from other instance connected to router"
read -p ""
echo "We can't connect to instance due to 'deny' policy."
read -p "We will request connectivity for database app."
pe "cat vpn-vpc-label-acl.yaml"
read -p ""
clear
read -p "Running AWI CLI ..."
pe "./awi connect --connection-config vpn-vpc-label-acl.yaml"
echo "We will try again to reach the EC2 instance with requested label."
pe "./awi list instance --cloud aws --tag 'staging*'"
read -p ""
echo "Connections have been established."
echo "Now we will try to reach the EC2 instance without requested label."
read -p ""
echo "We can't connect to instance due to 'deny' policy."
read -p ""
